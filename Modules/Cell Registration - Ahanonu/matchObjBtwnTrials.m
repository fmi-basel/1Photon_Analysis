function [OutStruct] = matchObjBtwnTrials(inputImages, max_Distance, reference_Session, ...
    analysis_Type, runMotionCorrection, imageCorr, imageCorrType)
	...
	options.runMotionCorrection = runMotionCorrection;
...
function [OutStruct] = computeGlobalIdsPairwise(OutStruct,coords,options,nSessions,inputImages,inputImagesOriginal)
	...
	% initialize global ref matrix
	% OutStruct.globalIDs(:,1) = 1:size(coordsGlobal,1);
	OutStruct.globalIDs(:,options.trialToAlign) = 1:size(coords{options.trialToAlign}, 1);
	...
	for i = 1:nSessions
		if i==options.trialToAlign
			OutStruct.globalIDs(1:size(coords{options.trialToAlign}, 1),i) ...
                = 1:size(coords{options.trialToAlign}, 1);
...
function [inputImages] = checkImageDimensions(inputImages)
    ...
    dimsList = [cell2mat(cellfun(@(c) size(c, 1), inputImages, 'UniformOutput', false)); ...
        cell2mat(cellfun(@(c) size(c, 2), inputImages, 'UniformOutput', false)); ...
        cell2mat(cellfun(@(c) size(c, 3), inputImages, 'UniformOutput', false))]';	
